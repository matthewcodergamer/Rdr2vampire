...
bool FeedingController::Initialize() {
    ...
    logger_.Write(util::LogLevel::Info, "FeedingController initialized; internal resource has no player HUD.");
    return true;
}
...
void FeedingController::Update(const core::FrameContext& frame) {
    ...
    if (mode_ == FeedMode::Drain && target_ != 0 && !gameApi_.PedAlive(target_) && state_ == FeedingState::FeedLoop) {
        ApplyCompletion();
...
bool FeedingController::AcquireCandidate(std::uint64_t nowMs) noexcept {
    ...
    if (!ValidateParticipants(reason)) {
        if (config_.debug.enabled && reason) logger_.Write(util::LogLevel::Debug, std::string("Feed target rejected: ") + reason);
        target_ = 0;
        return false;
    }
...
void FeedingController::Enter(FeedingState state, std::uint64_t nowMs) noexcept {
    ...
    if (config_.debug.enabled) logger_.Write(util::LogLevel::Debug, std::string("Feeding state -> ") + StateName(state));
}
...
void FeedingController::BeginGrab() noexcept {
    if (mode_ == FeedMode::Drain && feedingApi_.IsHuman(target_)) {
        if (feedingApi_.StartGrapple(player_, target_)) {
            playerTaskOwned_ = true;
            targetTaskOwned_ = true;
            return;
        }
    ...
    feedingApi_.StandStill(player_, config_.feeding.grabMs + std::max(config_.feeding.sipDurationMs, config_.feeding.drainDurationMs));
    feedingApi_.StandStill(target_, config_.feeding.grabMs + std::max(config_.feeding.sipDurationMs, config_.feeding.drainDurationMs));
    playerTaskOwned_ = true;
    targetTaskOwned_ = true;
}
...
void FeedingController::Abort(std::string_view reason) noexcept {
    if (state_ == FeedingState::Idle) return;
    if (config_.debug.enabled) logger_.Write(util::LogLevel::Debug, std::string("Feed aborted: ") + std::string(reason));
    CleanupOwnedTasks();
    ResetInteraction();
}

void FeedingController::CleanupOwnedTasks() noexcept {
    if (playerTaskOwned_ && player_ != 0 && gameApi_.EntityExists(player_)) feedingApi_.ClearTasks(player_);
    if (targetTaskOwned_ && target_ != 0 && gameApi_.EntityExists(target_)) feedingApi_.ClearTasks(target_);
