...
enum class CatalogLoadResult {
    Missing,
    Loaded,
    Rejected,
};

CatalogLoadResult ReadCatalogFile(const std::filesystem::path& path,
    ...
    if (!ParseNarrativeScript(buffer.str(), output, [&logger, label](std::string_view message) {
        ...
        logger.Write(util::LogLevel::Warning,
                     std::string(label) + " was rejected; existing dialogue remains active.");
...
void NarrativeController::LoadScript(const std::filesystem::path& path) {
    ...
    try {
        ...
        const auto primaryResult = ReadCatalogFile(path, primary, logger_, "Nightwalker.dialogue");
        ...
        const auto supplementalPath = path.parent_path() / L"Nightwalker.voice.dialogue";
        NarrativeCatalog supplemental{};
        if (ReadCatalogFile(supplementalPath, supplemental, logger_,
                            "Nightwalker.voice.dialogue") == CatalogLoadResult::Loaded) {
            AppendSupplement(catalog_, std::move(supplemental), logger_);
        }
    } catch (...) {
...
bool NarrativeController::StartResolvedSequence(const NarrativeSequence& sequence,
    ...
    if (!playback_.Start(sequence, nowMs,
                         static_cast<std::uint64_t>(config_.narrative.maxSequenceMs))) {
        return false;
    }
...
bool NarrativeController::StartSequenceFamily(std::string_view familyId,
    ...
    if (!sequence) {
        logger_.Write(util::LogLevel::Warning,
                      std::string("Narrative sequence family missing: ") +
                          std::string(familyId));
        return false;
    }
    return StartResolvedSequence(*sequence, nowMs);
...
void NarrativeController::DrawCurrentSubtitle() noexcept {
    ...
    textApi_.Rectangle(0.5F, centerY, boxWidth, boxHeight, 8, 8, 8, 118);
    float y = top + 0.006F;
    if (speaker) {
