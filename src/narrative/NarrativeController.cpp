...
CatalogLoadResult ReadCatalog(const std::filesystem::path& path,
    ...
    buffer << input.rdbuf();
    if (!ParseNarrativeScript(buffer.str(), output,
                              [&logger, label](std::string_view message) {
...
void NarrativeController::Update(const core::FrameContext& frame) {
    ...
    const std::string before = playback_.CurrentLine()
        ? playback_.CurrentLine()->id
        : std::string{};
...
void NarrativeController::DrawCurrentSubtitle() noexcept {
    ...
    float y = top + 0.006F;
    if (speaker) {
        textApi_.CenteredText(line->speaker.c_str(), 0.5F, y, 0.31F,
                              216, 208, 194, 240);
        y += lineStep;
    }
    for (const auto& text : wrapped) {
        textApi_.CenteredText(text.c_str(), 0.5F, y, 0.37F,
                              235, 231, 220, 245);
        y += lineStep;
    }
}
