...
ReactiveDialogueEvent WeaponTransitionEvent(game::PlayerWeaponKind kind) noexcept {
    switch (kind) {
        ...
        default: return ReactiveDialogueEvent::None;
    }
}
} // namespace
...
ReactiveDialogueEvent ReactiveDialogueModel::Update(const ReactiveDialogueInput& in) noexcept {
    ...
    if (in.shooting && !shooting_) {
        shooting_ = true;
        shotPending_ = true;
        shotDeadlineMs_ = in.nowMs + kShotClassifyMs;
        aimed_ = in.aimed;
    ...
    if (!weaponInitialized_) {
        weaponInitialized_ = true;
        weaponKind_ = in.weaponKind;
    } else if (in.weaponKind != game::PlayerWeaponKind::Unknown && in.weaponKind != weaponKind_) {
        const auto previous = weaponKind_;
        weaponKind_ = in.weaponKind;
        ...
        const auto event = WeaponTransitionEvent(in.weaponKind);
        if (event != ReactiveDialogueEvent::None) return event;
    }

    if (in.aimed && !aimed_) {
        aimed_ = true;
        aimHeldEmitted_ = false;
        aimStartedMs_ = in.nowMs;
        return ReactiveDialogueEvent::AimStarted;
    }
    if (in.aimed && aimed_ && !aimHeldEmitted_ && in.nowMs - aimStartedMs_ >= kAimHoldMs) {
        aimHeldEmitted_ = true;
        return ReactiveDialogueEvent::AimHeld;
    }
    if (!in.aimed && aimed_) {
        aimed_ = false;
        aimHeldEmitted_ = false;
        aimStartedMs_ = 0;
        return ReactiveDialogueEvent::AimLowered;
    }

    if (in.distanceToBoss >= 0.0F) {
        if (!proximityInitialized_) {
            proximityInitialized_ = true;
            closeRange_ = in.distanceToBoss <= kCloseApproachMeters;
        } else if (!closeRange_ && in.distanceToBoss <= kCloseApproachMeters) {
            closeRange_ = true;
            return ReactiveDialogueEvent::CloseApproach;
        } else if (closeRange_ && in.distanceToBoss >= kBackAwayMeters) {
            closeRange_ = false;
            return ReactiveDialogueEvent::BackedAway;
        }
    }
    return ReactiveDialogueEvent::None;
}

void ReactiveDialogueModel::Reset() noexcept {
    weaponKind_ = game::PlayerWeaponKind::Unknown;
    weaponInitialized_ = false;
    aimed_ = false;
    shooting_ = false;
    aimHeldEmitted_ = false;
    shotPending_ = false;
    meleeEngaged_ = false;
    meleeHitLatched_ = false;
    proximityInitialized_ = false;
    closeRange_ = false;
    aimStartedMs_ = 0;
    shotDeadlineMs_ = 0;
}
