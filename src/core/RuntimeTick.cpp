...
void Runtime::ReloadConfig(){
 bossHudController_.Cancel();vampireCombatController_.Cancel();movementController_.Cancel();vampireAiController_.Cancel();saintDenisDirector_.Cancel();feedingController_.Cancel();shadowstepController_.Cancel();progressionController_.Cancel();
 bool wasDebug=config_.debug.enabled;const auto iniPath=gameContext_.PluginDirectory()/L"Nightwalker.ini";config_=Config::Load(iniPath,[this](std::string_view m){logger_.Write(util::LogLevel::Warning,m);});systems::LoadSaintDenisSettings(iniPath,config_.encounter,[this](std::string_view m){logger_.Write(util::LogLevel::Warning,m);});progressionController_.ApplyAfterConfigReload();logger_.SetMinimumLevel(config_.debug.enabled?util::LogLevel::Debug:util::LogLevel::Info);debugInput_.Configure(config_.debug);shadowstepController_.ReloadPresentationSettings(iniPath);vampireAiController_.ReloadPresentationSettings(iniPath);if(wasDebug&&!config_.debug.enabled)debugVampireSpawner_.RequestDespawn();logger_.Write(util::LogLevel::Info,"Configuration reloaded; progression tuning reapplied after safe cleanup.");
}
