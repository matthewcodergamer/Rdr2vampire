...
void Runtime::Tick(){
 ...
 if(!debugDebounce_.IsArmed()||debugDebounce_.HasElapsed(now)){auto a=debugInput_.Poll();if(a.spawnTestPed){debugVampireSpawner_.RequestSpawn(now);debugDebounce_.Arm(now,250);}else if(a.despawnTestPed){debugVampireSpawner_.RequestDespawn();debugDebounce_.Arm(now,250);}else if(a.reloadConfig){ReloadConfig();debugDebounce_.Arm(now,250);}else if(a.restoreState){logger_.Write(util::LogLevel::Info,"Debug cleanup requested.");CancelSystems();RestoreOwnedState("debug cleanup");debugDebounce_.Arm(now,250);}}
 FrameContext frame{now,dt};for(auto* system:systems_)if(system)system->Update(frame);
}

void Runtime::ReloadConfig(){
 bool wasDebug=config_.debug.enabled;config_=Config::Load(gameContext_.PluginDirectory()/L"Nightwalker.ini",[this](std::string_view m){logger_.Write(util::LogLevel::Warning,m);});logger_.SetMinimumLevel(config_.debug.enabled?util::LogLevel::Debug:util::LogLevel::Info);debugInput_.Configure(config_.debug);if(wasDebug&&!config_.debug.enabled)debugVampireSpawner_.RequestDespawn();logger_.Write(util::LogLevel::Info,"Configuration reloaded.");
}
