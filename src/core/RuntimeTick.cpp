...
void Runtime::Tick(){
 ...
 if(!debugDebounce_.IsArmed()||debugDebounce_.HasElapsed(now)){auto a=debugInput_.Poll();if(a.shadowstepForward){shadowstepController_.RequestForward(now);debugDebounce_.Arm(now,250);}else if(a.spawnTestPed){debugVampireSpawner_.RequestSpawn(now);debugDebounce_.Arm(now,250);}else if(a.despawnTestPed){vampireAiController_.Cancel();debugVampireSpawner_.RequestDespawn();debugDebounce_.Arm(now,250);}else if(a.reloadConfig){ReloadConfig();debugDebounce_.Arm(now,250);}else if(a.restoreState){logger_.Write(util::LogLevel::Info,"Debug cleanup requested.");CancelSystems();RestoreOwnedState("debug cleanup");debugDebounce_.Arm(now,250);}}
 FrameContext frame{now,dt};for(auto* system:systems_)if(system)system->Update(frame);
}

void Runtime::ReloadConfig(){
 vampireAiController_.Cancel();shadowstepController_.Cancel();
