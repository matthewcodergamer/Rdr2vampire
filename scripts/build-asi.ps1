...
function Install-OfficialSdk {
    ...
    try {
        ...
        # The official SDK normally keeps its headers together. Copy the whole header directory
        # so relative includes continue to work. If natives.h lives elsewhere, copy its directory too.
...
try {
    ...
    if (!$msbuild) { throw 'MSBuild was not found. Install Visual Studio 2022 Desktop C++ workload or run through GitHub Actions.' }
    ...
    Write-Host "Nightwalker.asi ready: $asi"
